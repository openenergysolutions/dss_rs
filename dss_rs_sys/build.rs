...
fn main() {
    ...
    klusolvex_config.define("USE_SYSTEM_SUITESPARSE", "OFF"); // FIXME: should be "ON", but it causes failure..
    ...
    println!("cargo:include=dss_capi/klusolvex/include");
    println!("cargo:rustc-link-search=./dss_capi/lib/linux_x64");
    println!(concat!("cargo:rustc-link-lib=dss_capi"));
    println!(concat!("cargo:rustc-link-lib=klusolvex"));
}
